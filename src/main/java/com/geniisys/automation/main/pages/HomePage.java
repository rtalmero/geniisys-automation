...
public class HomePage {
	...
	private By homeBtnLocator = By.xpath("//a[@id='footerHome']");
	...
	public MarketingHomePage goToMarketingPage() {
		goToMainPage(marketingBtnLocator);
		return new MarketingHomePage(driver);
	}
	
	public void goToMainPage(By genericLocator) {
		...
		try {
			String genericMainPage = (driver.findElement(genericLocator)).getAttribute("title");
			driver.findClickableElement(genericLocator).click();
			LOGGER.info("Go to " + genericMainPage + " main page");
		} catch (TimeoutException e) {
			LOGGER.error(e);
		}
